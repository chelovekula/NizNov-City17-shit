...
public class Gradient {
    ...
    private HashMap<Integer, ColorInterval> generateColorIntervals() {
        ...
        if (this.mStartPoints[0] != 0.0f) {
            int argb = Color.argb(0, Color.red(this.mColors[0]), Color.green(this.mColors[0]), Color.blue(this.mColors[0]));
            Integer valueOf = Integer.valueOf(0);
            ColorInterval colorInterval = new ColorInterval(argb, this.mColors[0], ((float) this.mColorMapSize) * this.mStartPoints[0]);
            hashMap.put(valueOf, colorInterval);
        }
        for (int i = 1; i < this.mColors.length; i++) {
            int i2 = i - 1;
            Integer valueOf2 = Integer.valueOf((int) (((float) this.mColorMapSize) * this.mStartPoints[i2]));
            int[] iArr = this.mColors;
            int i3 = iArr[i2];
            int i4 = iArr[i];
            float f = (float) this.mColorMapSize;
            float[] fArr = this.mStartPoints;
            ColorInterval colorInterval2 = new ColorInterval(i3, i4, (fArr[i] - fArr[i2]) * f);
            hashMap.put(valueOf2, colorInterval2);
        }
        float[] fArr2 = this.mStartPoints;
        if (fArr2[fArr2.length - 1] != 1.0f) {
            int length = fArr2.length - 1;
            Integer valueOf3 = Integer.valueOf((int) (((float) this.mColorMapSize) * fArr2[length]));
            int[] iArr2 = this.mColors;
            ColorInterval colorInterval3 = new ColorInterval(iArr2[length], iArr2[length], ((float) this.mColorMapSize) * (1.0f - this.mStartPoints[length]));
            hashMap.put(valueOf3, colorInterval3);
        }
        return hashMap;
    }
    ...
    public int[] generateColorMap(double d) {
        ...
        for (int i2 = 0; i2 < this.mColorMapSize; i2++) {
            if (generateColorIntervals.containsKey(Integer.valueOf(i2))) {
                colorInterval = (ColorInterval) generateColorIntervals.get(Integer.valueOf(i2));
                i = i2;
            }
