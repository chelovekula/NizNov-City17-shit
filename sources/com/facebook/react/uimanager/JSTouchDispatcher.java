...
public class JSTouchDispatcher {
    ...
    public void handleTouchEvent(MotionEvent motionEvent, EventDispatcher eventDispatcher) {
        ...
        if (action == 0) {
            if (this.mTargetTag != -1) {
                FLog.m50e(str, "Got DOWN touch before receiving UP or CANCEL from last gesture");
            }
            ...
            if (i2 == -1) {
                FLog.m50e(str, "Unexpected state: received touch event but didn't get starting ACTION_DOWN for this gesture before");
            } else if (action == 1) {
                ...
                if (this.mTouchEventCoalescingKeyHelper.hasCoalescingKey(motionEvent.getDownTime())) {
                    ...
                    FLog.m50e(str, "Received an ACTION_CANCEL touch event for which we have no corresponding ACTION_DOWN");
                }
                this.mTargetTag = -1;
                this.mGestureStartTime = Long.MIN_VALUE;
            } else {
    ...
    private void dispatchCancelEvent(MotionEvent motionEvent, EventDispatcher eventDispatcher) {
        if (this.mTargetTag == -1) {
            FLog.m90w(ReactConstants.TAG, "Can't cancel already finished gesture. Is a child View trying to start a gesture from an UP/CANCEL event?");
            return;
        }
