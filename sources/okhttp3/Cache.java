...
public final class Cache implements Closeable, Flushable {
    ...
    private static final class Entry {
        ...
        private void writeCertList(BufferedSink bufferedSink, List<Certificate> list) throws IOException {
            try {
                ...
                for (int i = 0; i < size; i++) {
                    bufferedSink.writeUtf8(ByteString.m177of(((Certificate) list.get(i)).getEncoded()).base64()).writeByte(10);
                }
        ...
        public boolean matches(Request request, C1069Response response) {
            return this.url.equals(request.url().toString()) && this.requestMethod.equals(request.method()) && HttpHeaders.varyMatches(response, this.varyHeaders, request);
        }

        public C1069Response response(Snapshot snapshot) {
            String str = this.responseHeaders.get("Content-Type");
            String str2 = this.responseHeaders.get("Content-Length");
            return new C1069Response.Builder().request(new Request.Builder().url(this.url).method(this.requestMethod, null).headers(this.varyHeaders).build()).protocol(this.protocol).code(this.code).message(this.message).headers(this.responseHeaders).body(new CacheResponseBody(snapshot, str, str2)).handshake(this.handshake).sentRequestAtMillis(this.sentRequestMillis).receivedResponseAtMillis(this.receivedResponseMillis).build();
        }
    }
    ...
    Cache(File file, long j, FileSystem fileSystem) {
        this.internalCache = new InternalCache() {
            public C1069Response get(Request request) throws IOException {
                return Cache.this.get(request);
            }

            public CacheRequest put(C1069Response response) throws IOException {
            ...
            public void update(C1069Response response, C1069Response response2) {
    ...
    @Nullable
    public C1069Response get(Request request) {
        try {
            ...
            try {
                ...
                C1069Response response = entry.response(snapshot);
    ...
    /* access modifiers changed from: 0000 */
    @Nullable
    public CacheRequest put(C1069Response response) {
        Editor editor;
