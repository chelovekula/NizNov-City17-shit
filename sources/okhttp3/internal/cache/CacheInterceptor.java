...
public final class CacheInterceptor implements Interceptor {
    ...
    public C1069Response intercept(Chain chain) throws IOException {
        ...
        C1069Response response = internalCache != null ? internalCache.get(chain.request()) : null;
        CacheStrategy cacheStrategy = new Factory(System.currentTimeMillis(), chain.request(), response).get();
        Request request = cacheStrategy.networkRequest;
        C1069Response response2 = cacheStrategy.cacheResponse;
        ...
        try {
            C1069Response proceed = chain.proceed(request);
            if (proceed == null && response != null) {
            }
            if (response2 != null) {
                if (proceed.code() == 304) {
                    C1069Response build = response2.newBuilder().headers(combine(response2.headers(), proceed.headers())).sentRequestAtMillis(proceed.sentRequestAtMillis()).receivedResponseAtMillis(proceed.receivedResponseAtMillis()).cacheResponse(stripBody(response2)).networkResponse(stripBody(proceed)).build();
                    proceed.body().close();
            ...
            C1069Response build2 = proceed.newBuilder().cacheResponse(stripBody(response2)).networkResponse(stripBody(proceed)).build();
            if (this.cache != null) {
    ...
    private static C1069Response stripBody(C1069Response response) {
        return (response == null || response.body() == null) ? response : response.newBuilder().body(null).build();
    }

    private C1069Response cacheWritingResponse(final CacheRequest cacheRequest, C1069Response response) throws IOException {
